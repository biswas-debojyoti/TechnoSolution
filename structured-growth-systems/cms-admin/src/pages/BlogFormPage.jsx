...
import { ArrowLeft, Upload, X, ImageIcon, Save, Eye, Loader2 } from 'lucide-react'
...
export default function BlogFormPage() {
  ...
  const [form, setForm] = useState({ heading: '', subHeading: '', status: 'draft' })
  ...
  useEffect(() => {
    if (blog && isEdit) {
      setForm({ heading: blog.heading || '', subHeading: blog.subHeading || '', status: blog.status || 'draft' })
  ...
  const handleImageChange = (e) => {
    ...
    if (!file) return
    if (file.size > MAX_SIZE) { toast.error('Image must be under 5MB.'); return }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-6 h-14 border-b border-[var(--border)] bg-[var(--bg-surface)] shrink-0">
        <div className="flex items-center gap-3">
          <Link to="/blogs" className="p-1.5 rounded-sm text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)]">
            <ArrowLeft size={15} />
          </Link>
          <div className="w-px h-4 bg-[var(--border)]" />
          <h1 className="text-sm font-semibold text-[var(--text-primary)]" style={{ fontFamily: 'Syne, sans-serif' }}>
            {isEdit ? 'Edit Blog Post' : 'New Blog Post'}
          </h1>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => handleSave('draft')} disabled={saving} className="btn-ghost text-xs py-1.5 px-3">
            {saving ? <Spinner size={12} /> : <Save size={12} />}
            Save draft
          </button>
          <button onClick={() => handleSave('published')} disabled={saving} className="btn-primary text-xs py-1.5 px-3">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mb-4">
            ...
            />
            {errors.heading && <p className="text-xs text-red-400 mt-1">{errors.heading}</p>}
          </div>
                ...
                {['draft', 'published'].map(s => (
                  <button key={s} onClick={() => setForm(p => ({ ...p, status: s }))}
                    className={`flex-1 py-1.5 text-xs rounded-sm border transition-colors capitalize
                      ${form.status === s
                        ? s === 'published' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-[var(--bg-hover)] border-[var(--border-light)] text-[var(--text-primary)]'
            ...
            <div>
              ...
              {imagePreview ? (
                ...
                <button onClick={() => fileRef.current?.click()}
                  ...
                  <span className="text-xs text-[var(--text-muted)] opacity-60">JPEG, PNG, WEBP · max 5MB</span>
                </button>
              )}
            ...
            {isEdit && blog && (
              <div className="border-t border-[var(--border)] pt-4 space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-xs text-[var(--text-muted)] font-mono">{blog._id?.slice(-8)}</span>
                </div>
              </div>
            )}
          </div>
