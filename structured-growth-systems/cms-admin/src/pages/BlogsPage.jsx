...
export default function BlogsPage() {
  ...
  return (
    <PageShell
      ...
      actions={
        <>
          <SearchInput value={search} onChange={handleSearch} placeholder="Search blogs…" />
          <SelectFilter
            value={status}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[var(--border)] bg-[var(--bg-elevated)]">
              {['Post', 'Sub-heading', 'Status', 'Created', 'Actions'].map(h => (
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={5}>
                <Empty
                  icon={FileText}
                  message={search || status ? 'No blogs match your filters.' : 'No blog posts yet.'}
                  action={<Link to="/blogs/new" className="btn-primary text-xs">Create first post</Link>}
                />
              </td></tr>
            ) : blogs.map(blog => (
              <tr key={blog._id} className="border-b border-[var(--border)] hover:bg-[var(--bg-hover)] transition-colors">
                {/* Title + image */}
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <BlogImage blog={blog} />
                    <div className="min-w-0">
                      <p className="text-[var(--text-primary)] font-medium truncate max-w-[220px]">{blog.heading}</p>
                      <p className="text-xs text-[var(--text-muted)] font-mono">{blog._id.slice(-6)}</p>
                    </div>
                  </div>
                </td>
                {/* Subheading */}
                <td className="px-4 py-3 text-[var(--text-secondary)] max-w-[180px]">
                  <span className="truncate block">{blog.subHeading || '—'}</span>
                </td>
                {/* Status */}
