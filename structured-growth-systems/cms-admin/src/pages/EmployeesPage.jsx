...
import { Plus, Check, X, Shield, Pencil, Image as ImageIcon, ToggleLeft, ToggleRight } from 'lucide-react'
...
export default function EmployeesPage() {
  ...
  const { employees, pagination, isLoading, error, mutate } = useEmployees({ page, limit: 10 })
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-display font-bold text-[var(--text-primary)]">Employees</h1>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {isLoading ? (
                <tr>
                  <td colSpan="6" className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                       ...
                       <span className="text-[var(--text-muted)]">Fetching records...</span>
                    </div>
                  </td>
                </tr>
              ) : employees.length === 0 ? (
                ...
                employees.map((emp) => (
                  <tr key={emp._id} className="hover:bg-[var(--bg-elevated)]/30 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-[var(--bg-surface)] border border-[var(--border)] flex items-center justify-center overflow-hidden shrink-0">
                          {emp.image ? (
                            <img src={employeeApi.imageUrl(emp._id)} alt={emp.name} className="w-full h-full object-cover" />
                          ) : (
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-[var(--text-muted)] uppercase tracking-tight">Joined {new Date(emp.joiningDate).toLocaleDateString()}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <p className="text-[var(--text-primary)]">{emp.contactNo}</p>
                      <p className="text-xs text-[var(--text-muted)]">{emp.whatsappNo || 'No WhatsApp'}</p>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        <p className="text-xs font-mono text-[var(--text-secondary)]">{emp.userId}</p>
                        <div className="flex flex-wrap gap-1 mt-1">
                          {emp.permissions?.map(p => (
                            <span key={p} className="px-1.5 py-0.5 rounded bg-blue-500/5 text-blue-400 border border-blue-500/20 text-[9px] font-mono">
                              {p}
                            </span>
                          ))}
                        </div>
                    ...
                    <td className="px-6 py-4 text-center">
                      ...
                      >
                        {emp.status === 'active' ? <Check size={12} /> : <X size={12} />}
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          className={`p-1.5 rounded-md transition-colors ${
                             emp.status === 'active' ? 'text-green-500' : 'text-[var(--text-muted)]'
